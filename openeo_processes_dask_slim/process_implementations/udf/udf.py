...
def run_udf(
    data: RasterCube, udf: str, runtime: str, context: Optional[dict] = None
) -> RasterCube:
    ...
    if input_was_dataset:
        ...
        attrs = data.attrs if isinstance(data, xr.DataArray) else {}

    udf_input = XarrayDataCube(data)
    ...
    if not result_array.attrs:
        result_array.attrs = attrs

    if input_was_dataset:
        result_array = _unstack_bands(result_array)
